...
def eq_regressao_logistica():
    ...
    coeficiente_tdb = 2.2588
    coeficiente_tr = 2.2588
    coeficiente_v = -37.9556
    coeficiente_rh = 0.1564
    coeficiente_met = 2.6392
    coeficiente_clo = 48.0170
    coeficiente_v_r = 6.4190
    coeficiente_clo_d = -19.8222
    intercept = -145.2763
    ...
    # Equação
    eq = intercept + coeficiente_tdb * tdb + coeficiente_tr * tr + coeficiente_v * v + coeficiente_rh * rh + \
         coeficiente_met * met + coeficiente_clo * clo + coeficiente_v_r * v_r + coeficiente_clo_d * clo_d
...
def ml_regressao_logistica():
    ...
    lab1 = {'tdb': tdb, 'tr': tr, 'rh': rh, 'v': v, 'met': met, 'clo': clo, 'v_r': v_r, 'clo_d': clo_d}
    dft1 = pd.DataFrame(data=lab1, index=[0])

    # Retorna o resultado do machine learning
    resultado = model.predict(dft1)
...
#############################################################################
# Lê o arquivo de aprendizado
model = load('model_rl.joblib')
